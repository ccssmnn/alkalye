...
describe("agents/automation", () => {
	it("builds stable selectors", () => {
		expect(makeAgentTestSelector(AGENTS_STABLE_IDS.submit)).toBe('[data-testid="agents-submit"]')
	...
	it("creates plan for action and fields", () => {
		...
		expect(plan).toEqual([
			{ action: "set-value", selector: '[data-testid="agents-action-select"]', value: "getDoc" },
			{ action: "set-value", selector: '[data-testid="agents-field-docId"]', value: "doc-1" },
			{ action: "click", selector: '[data-testid="agents-submit"]' },
		])
	})
})
